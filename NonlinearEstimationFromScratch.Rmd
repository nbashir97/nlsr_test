...
to make a number of assumptions I consider to be tenuous and the truly Byzantine
notation used by most authors.
...
in symbols and, where possible, avoided Greek symbols, special letter markings, super- and sub-scripts 
and other notations that are awkward to enter and print. This is certain to offend at least
one group of workers. I nevertheless hope my exposition is readable.
...
and columns being quantities observed. Sometimes we call these **variables**, but
some of the columns may be tags that are, in fact, part of the metadata. One of
the most common situations has a column of $M$, call it $m$, as the quantity we
wish to predict, or model, based on one or several of the other columns.
...
process or mistakes in its application.
...
I) almost always assume that the center, generally the mean, of the distribution
is 0. We write
...
but that is but one possible pattern.
...
injuries to driving while under the influence of alcohol when there was not
actual documentation of this.
...
were wage employees with a "triple time on Sunday" clause. 
...
- I knew that $log_{10}(4) \approx 0.6021$ (This knowledge was surely an
  ...
  At the time I knew a fair number of entries in log tables.)
- $20 * 0.6 + 20 * (-1) = -8$ is a good approximation to $log(0.4^{20}) \approx -7.9588$

It turned out the data were data converted from annual to quarterly figures and
...
of the choice of the model form. This may not be critical is we want only to provide
...
applications over the range and domain of the data used in the estimation. It may be 
dangerous to apply outside of this domain.
...
difficulty is not in the fit but in the understanding of the meaning of the 
parameter values.
...
stabilized (that is, using ideas of @marquardt1963) algorithms to avoid some
...
In this situation, we have found these two viewpoints do not mesh well.
...
This is still far from perfect, but does avoid some issues, though it may cause
programs to keep trying to improve parameter values to seek minima or maxima much
...
used the residual sum of squares as the basis for estimates of error around
...
hence suggest t-values for the parameters to judge their reliability. Truthfully,
for nonlinear models, it is misguided to trust such quantities too far.
...
"near" to the proposed set of parameters.  (?? a reference here would be useful)
...
form would be useful. This exercise will be made more difficult by the power of
...
to use known approaches to explore these violations of assumptions.
